...
const Profile = (): any => {
  ...
  return (
    ...
    {(data[0].slug === 'personal_info' ) ? <PersonalInfoPage />
         : (data[0].slug === 'account' ) ? <AccountPage />
         : (data[0].slug === 'bookings' ) ? <BookingsPage />
         : (data[0].slug === 'wallet' ) ? <Wallet />
         : (data[0].slug === 'banking' ) ? <BankingPage />
         : (data[0].slug === 'help_centre' ) ? <HelpCenterPage />
         : <></>}
    </>
  )
}
